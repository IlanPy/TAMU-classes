...
__global__ void jacobi_kernel(int n, double h, double *u, double *u_new, double *max_error) {
    __shared__ double tile[MAX_BLOCK_SIZE][MAX_BLOCK_SIZE];
    ...
    if (threadIdx.x == 0) tile[tx - 1][ty] = u[(i-1)*n + j];
    if (threadIdx.x == blockDim.x - 1) tile[tx + 1][ty] = u[(i+1)*n + j];
    if (threadIdx.y == 0) tile[tx][ty - 1] = u[i*n + (j-1)];
    if (threadIdx.y == blockDim.y - 1) tile[tx][ty + 1] = u[i*n + (j+1)];
    ...
    if(i < n-1 && j < n-1){
        x = i * h;
        y = j * h;
        double new_val = 0.25 * (tile[tx + 1][ty] + tile[tx - 1][ty] 
                + tile[tx][ty + 1] + tile[tx][ty - 1] - h*h*f(x, y));
                
        error = fabs(new_val - tile[tx][ty]); // u[i*n + j]
...
int main() {
    for (int iter = 1; iter < 6; iter++) {
        ...
        save_output(u, n, "gpu_shared_output.txt");

...
void file_thread_function (BoundedBuffer* request_buffer, FIFORequestChannel* chan, int m, string f/* add necessary arguments */) {
...
void worker_thread_function (BoundedBuffer* request_buffer, BoundedBuffer* response_buffer, 
        FIFORequestChannel* chan, int m, bool isFile, string f/* add necessary arguments */) {
...
void histogram_thread_function (BoundedBuffer* response_buffer, HistogramCollection* hc/* add necessary arguments */) {
    ...
    while(true){
        ...
        hc->update(data.person, data.time, data.ecgData);
    }
}

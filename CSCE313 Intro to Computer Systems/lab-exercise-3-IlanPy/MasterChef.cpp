...
void RemoveDepHandler(int sig) {
	...
	for(int i = 0; (long unsigned int)i < completedSteps->size(); i++){
		recipeSteps->RemoveDependency(completedSteps->at(i));
	}
	completedSteps->clear(); // Added to reset for next run
	/* End Section - 3 */
}
...
int main(int argc, char **argv)
{
	...
	while(completeCount != recipeSteps->Count()){
		...
		for(int i = 0; (long unsigned int)i < ready_steps.size(); i++){
			if(!ready_steps.at(i)->running){
				ready_steps.at(i)->running = true;
				makeTimer(ready_steps.at(i), ready_steps.at(i)->duration);
			}
		}
	}
	/* End Section - 1 */

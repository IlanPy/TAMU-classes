    ...
    for (int ttl = 1; ttl <= MAX_HOPS; ) {
        ...
        while (1) {
            ...
            if (ret == 0) {
                ...
                int bytes = recvfrom(sockfd, recv_buf, PACKET_LEN, 0, (sockaddr *)&info, &info_size);
                if (bytes < 0)
                {
                ...
                if(bytes >= 2 * (sizeof(ipheader) + sizeof(icmpheader))){
                    icmpheader* icmp_router = (icmpheader*)(recv_buf + sizeof(ipheader));
                    if(icmp_router->icmp_type == ICMP_TIME_EXCEEDED){
                        icmpheader* icmp_original = (icmpheader*)(recv_buf + 2*sizeof(ipheader) + sizeof(icmpheader));
                        if(icmp_original->icmp_seq == htons(ttl) && icmp_original->icmp_id == htons(getpid())){
                            ipheader* ip_router = (ipheader*)recv_buf;
                            char *src_ip = inet_ntoa(ip_router->iph_sourceip);
                            printf("%s\n", src_ip);
                            ttl++;
                            break;
                        }
                    }
                } else if(bytes >= (sizeof(ipheader) + sizeof(icmpheader))){
                    icmpheader* icmp_router = (icmpheader*)(recv_buf + sizeof(ipheader));
                    if(icmp_router->icmp_type == ICMP_ECHO_REPLY && icmp_router->icmp_id == htons(getpid())){
                        ipheader* ip_router = (ipheader*)recv_buf;
                        char *src_ip = inet_ntoa(ip_router->iph_sourceip);
                        printf("%s\n", src_ip);
                        exit(0);

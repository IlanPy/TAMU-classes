...
int main (int argc, char* argv[]) {
	...
	while (true) {
		...
		int newsockfn = control_channel->accept_conn();

		// Create a new channel with the accepted socket
		TCPRequestChannel* channel = new TCPRequestChannel(newsockfn);

		// Dispatch a new thread to handle_process_loop
		// Since the main thread is not blocked, it can accept more connections
		thread(handle_process_loop, channel).detach();
	}
